               ...
               const uint8_t flags, const int max_phase4_progress_updates)
{
    ...
    ParkBits to_write_p7;
		...
		for( bool cont = true ; cont; ){
			cont = !res.table7_sm->CurrentBucketIsLast();
			...
			std::thread p7_thread( [&k, &pos_size, &res, &right_entry_size_bytes, &P7_entry_buf,
						...
						for( uint64_t f7_position = 0; f7_position < res.table7_sm->CurrentBucketSize(); f7_position += right_entry_size_bytes ) {
							...
							if( ( res.table7_sm->CurrentBucketNo() > 0 || f7_position > 0 )
									&& ((f7_position+res.table7_sm->CurrentBucketStart())/right_entry_size_bytes) % kEntriesPerPark == 0 ) {
			...
			std::thread C_thread([&res, &right_entry_size_bytes, &k, &C1_entry_buf, &final_file_writer_1,
						...
						for( uint64_t f7_position = 0; f7_position < res.table7_sm->CurrentBucketSize(); f7_position += right_entry_size_bytes ) {
							...
							if( ((f7_position+res.table7_sm->CurrentBucketStart())/right_entry_size_bytes) % kCheckpoint1Interval == 0) {
								...
								prev_y = entry_y;
								if( ((f7_position+res.table7_sm->CurrentBucketStart())/right_entry_size_bytes) % (kCheckpoint1Interval * kCheckpoint2Interval) == 0) {
			...
			p7_thread.join();
			C_thread.join();
			if( cont ) res.table7_sm->SwitchNextBucket();
		}
